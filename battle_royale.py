...
class BRFighter:
    """
    Defines one fighter in a battle royale
    simulator.
    """

    def __init__(self,name,startingHP,minAtt,maxAtt,fighterType):
        self.name = name
        self.hp = startingHP
        self.minAtt = minAtt
        self.maxAtt = maxAtt
    ...
    def attack(self,other):
        """
            Calculates damage for a single attack by self on other,
    ...
    def takeDamage(self,damage):
        """
            Reduces self's HP by damage. Returns true if HP has
            been reduced to <= 0.
        """
        self.hp -= damage
        return self.hp <= 0

def simulate(fighterList):
    """
        Has randomly-selected members of fighterList attack
        other randomly-selected members of figherList
        until there is only one member of fighterList
    """
    while len(fighterList) > 1:
        attIndex = random.randint(0,len(fighterList)-1)
        defIndex = random.randint(0,len(fighterList)-2)
        if defIndex >= attIndex:
            defIndex += 1
        if fighterList[attIndex].attack(fighterList[defIndex]):
